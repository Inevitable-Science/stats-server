...
export async function fetchAllTokenBalances(
  ...
  try {
    ...
    const response = await fetchWithRetry<AlchemyTokenBalancesSchemaType>(
      ALCHEMY_TOKEN_BAL_URL,
      payload
    );
...
export async function fetchTokenMetadata(
  ...
  try {
    ...
    if (!data.name || !data.symbol || !data.decimals)
      throw new Error(`Name, Symbol or Decimals missing in metadata response`);
