...
import FollowersModel, { FollowersDocument } from '../../../config/models/followers_schema';
...
async function fetchAndUpdateTwitterFollowers(): Promise<void> {
  try {
    ...
    await sendDiscordMessage(`**Starting daily twitter followers refresh at: ${new Date().toLocaleString()}**`);
    ...
    for (const foundDao of daos) {
      ...
      await sendDiscordMessage(`**Refreshed twitter followers for @${twitter_username} - Count: ${follower_count} at: ${new Date().toLocaleString()}**`);
      await sleep(5000);
    };
    ...
    console.log(err);
    await sendDiscordMessage(`**Failed to refresh twitter followers at: ${new Date().toLocaleString()}**`);
  };
};
